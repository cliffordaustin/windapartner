...
function PartnerSignin(props: PaperProps) {
  ...
  const invitationCode = router.query.invitation_code as string;

  const addAgentFromInviteView = async (token: string) => {
    await axios.post(
      `${process.env.NEXT_PUBLIC_baseURL}/add-agent-from-invite/`,
      {
  ...
  const addAgentFromInviteViewWithRedirect = async (token: string) => {
    await axios
      .post(
        `${process.env.NEXT_PUBLIC_baseURL}/add-agent-from-invite/`,
        {
  ...
  // useEffect(() => {
  //   if (token) {
  //     addAgentFromInviteViewWithRedirect(token);
  //   }
  // }, [token]);
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  try {
    const checkAgentInvitationCode = async () => {
      await axios.post(
        `${process.env.NEXT_PUBLIC_baseURL}/agent-access-by-email/check-invitation/`,
        {
          invitation_code: query.invitation_code,
    ...
    await checkAgentInvitationCode();
