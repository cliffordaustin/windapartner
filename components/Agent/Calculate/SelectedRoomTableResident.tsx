...
function SelectedRoomTableResident({
  room,
  guestType,
  agentRates,
  displayRackRates,
    ...
    () =>
      room.packages.map((packageName) => {
        const guestAvailability: RoomAvailabilityType[] =
          packageName.room_resident_availabilities.map((item) => {
            const guestAvailability =
              item.room_resident_guest_availabilities.find(
                (guest) =>
...
export default SelectedRoomTableResident;
